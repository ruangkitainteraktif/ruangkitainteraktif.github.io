/* ── VIIRS SNPP Time Slider — NASA GIBS WMTS ── */
(function () {
  'use strict';

  var LAYER_KEY = 'viirs-snpp';
  var GIBS_LAYER_ID = 'VIIRS_SNPP_CorrectedReflectance_TrueColor';
  var GIBS_EXT = 'jpeg';
  var DAY_COUNT = 30;
  ...
  function updateUrl(dateStr) {
    ...
    if (!layer) return;
    var newUrl = 'https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/' + GIBS_LAYER_ID + '/default/' + dateStr + '/GoogleMapsCompatible_Level9/{z}/{y}/{x}.' + GIBS_EXT;
    layer.setUrl(newUrl);
  }
  ...
  var SliderControl = L.Control.extend({
    ...
    onAdd: function () {
      ...
      if (typeof addUnifiedSlider !== 'function') {
        ...
        titleRow.textContent = 'VIIRS SNPP';
      }
  ...
  function showSlider() {
    if (!sliderControl) {
      ...
      if (typeof addUnifiedSlider === 'function') {
        addUnifiedSlider('viirs-snpp', 'VIIRS SNPP', el);
      } else {
  ...
  function hideSlider() {
    if (sliderControl) {
      if (typeof removeUnifiedSlider === 'function') removeUnifiedSlider('viirs-snpp');
  ...
  window.viirsSnppTimeSliderCleanup = cleanup;
})();
